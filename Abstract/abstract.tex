% ************************** Thesis Abstract *****************************
% Use `abstract' as an option in the document class to print only the titlepage and the abstract.
\begin{abstract}
Programming exercises used at school and in textbooks mostly focus on fundamental concepts, and lack reflections from real-world software development.
As a result, learners may not be able to achieve sufficient experience for practical software development from such learning materials.
To this end, I develop RealCode, a system that repurposes real issue and code diff data on GitHub to programming exercises.
Exercises in RealCode are intended to provide learning experience grounded in real-world projects.
I present my feasibility explorations, resulting in a decision tree for extracting GitHub issues appropriate for exercise use.
This thesis also reports the implementation of RealCode as well as the originality evaluation and informal qualitative study confirming unique benefits of RealCode exercises.
\end{abstract}
